<script lang="ts">
  ...
  const onMapClick = (e) => {
    ...
    if(!$isAreaConfirmed) {
      ...
      if(!$round && $gotInitialSeedFromUrl ) {
  ...
  const initializeMap = () => {
    leaflet.Icon.Default.prototype.options.imagePath = "./streetquiz/images/leaflet/";
    ...
    map = leaflet.map(mapElement, {
      ...
      .addControl(leaflet.control.zoom({
        position: 'topright',
        zoomInText: "&#43;" + (viewportWidth > 800 ? " " : ""),
        zoomOutText: "&minus;" + (viewportWidth > 800 ? " " : ""),
      }));
